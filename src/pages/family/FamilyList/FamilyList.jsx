...
function FamilyList() {
  ...
  const updateFamilyMembers = () => {
    if (familyData && currentUser) {
      const currentUserMember = familyData.familyDataList.find(
        (member) => member.userId === currentUser.userId
      );
      setCurrentUserData(currentUserMember);

      if (window.innerWidth <= 768) {
        const filteredFamilyMembers = familyData.familyDataList.filter(
          (member) => member.userId !== currentUser.userId
        );
